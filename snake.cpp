...
{
	length = 0;
	AddNode(width / 2, height / 2); //一开始 Snake 的位置在界面中间
	PutFood();
	maxlength = (width -1) * (height -1); //计算胜利的时候， Snake 的最大长度
	die = false;
	win = false;
...
void Snake::Judge(void)
{
	...
	if(SnakeNode[0].x == 0 || SnakeNode[0].x == width || SnakeNode[0].y == 0 || SnakeNode[0].y == height) {   //判断是否碰到边界
		die = true;
		return;
	}
...
void Snake::Move(void)
{
	...
	for(int i = length -1; i > 0; i--)
		SnakeNode[i] = SnakeNode[i -1]; //更新 Snake 结点的坐标
	switch(direction)
	{
		case 1:
			SnakeNode[0].y--; //up
