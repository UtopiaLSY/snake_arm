...
class Snake
{
	public:
		...
		void getCoordinate(vector<Node>&node, Node &food); /* 输出 Snake 的坐标和果实的坐标*/
	...
	private:
		...
		void AddNode(const int &w, const int &n); /* 增加 Snake 的节点即身体变长 */
		void PutFood(void); /* 随机放置果实的坐标 */
		vector<Node> SnakeNode; /* 保存 Snake 身体的每个节点坐标 */
		Node Food; /* 果实的坐标 */
		int direction; /* 1: up 2: right 3: down 4: left */
		int length; /* Snake 身体的长度 */
		int maxlength; /*Snake 的最大长度，等于这个长度就胜利 */
		bool die;
		bool win;
		/* Snake 移动的范围 */
		int height;
